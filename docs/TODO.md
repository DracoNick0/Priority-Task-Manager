...
- Expanded interactive console seam adoption to the remaining interactive handlers: `SettingsHandler`'s no-arg `defaults` menu now takes an `IInteractiveConsoleFacade` (previously used only static `Console.*`/`ConsoleMenuHelper.*` calls and had no interactive test coverage); `ListHandler`'s no-arg `list switch` menu was migrated off direct `Console.*` calls onto the same facade (its hand-rolled `DrawListMenu` renderer was replaced with the shared `DrawMenuItems`/`UpdateMenuSelection` facade calls); and a stray direct `Console.CursorVisible = true;` left over in `EventCommandHandler.HandleEditEvent` was fixed to use `_console`. Added `SettingsHandlerInteractiveSeamTests.cs` and extended `ListHandlerInteractiveSeamTests.cs` following the existing `FakeInteractiveConsoleFacade` pattern. Remaining direct-console interactive prompts (`AddHandler`'s no-arg task creation, `list delete`/`cleanup` confirm prompts) are simple linear confirm/read-line flows, not menu/cursor-driven, and are intentionally out of scope for the facade seam.
