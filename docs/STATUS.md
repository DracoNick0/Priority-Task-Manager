...
| Scheduling logic | 🟢 Integrated | The Command Center computes real schedules by sending Hive task/event data and the active list's effective settings to a separately-running `PriorityTaskManager.API` instance (`/api/local/schedule`), not mock data (tracked by issue #47). The client no longer spawns this process itself; the API must already be running (see [WORKFLOW.md](WORKFLOW.md)). |
...
| Networked API | 🔴 Not yet integrated | No login or API-backed repository yet; the client only reaches the local `/api/local/schedule` route on a separately-running API instance (tracked by issue #44). |
