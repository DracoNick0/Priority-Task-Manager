...
| Networked API | 🟡 Partially implemented | `PriorityTaskManager.API` has account/JWT auth (see [ARCHITECTURE_INTEGRATIONS.md](ARCHITECTURE_INTEGRATIONS.md)) and minimal REST endpoints for task/list/event CRUD wrapping core services; no client (CLI or Flutter) authenticates against it yet (tracked by issue #44). A separate unauthenticated `/api/local/schedule` route runs the real scheduling strategies statelessly for the offline Flutter client (see [ARCHITECTURE_INTEGRATIONS.md](ARCHITECTURE_INTEGRATIONS.md)). An authenticated, Subscription-tier-gated `/api/schedule` route (same stateless computation) also exists but is not yet called by any client in production use (Flutter's dev-only auto-login shim does call it for testing). `Account.SubscriptionTier` (`Free`/`Subscription`) is carried as a JWT claim; Development-environment startup seeds fixed `free@dev.local`/`subscriber@dev.local` test accounts (see `PriorityTaskManager.API/Dev/DevAccountSeeder.cs`). This login->JWT->authenticated-`/api/schedule` round trip has now been live-smoke-tested end to end against a real local Postgres instance (2026-08-30), including confirming Free-tier callers get `403 Forbidden`. New account registration (`POST /api/auth/register`) defaults to Subscription tier (not Free) during the MVP/beta grace period, config-driven via `BetaGracePeriod:DefaultNewAccountsToSubscription` (issue #50); the auth response carries a `BetaGracePeriodNotice` for a future client to display, though no client shows it yet. |
...
| Scheduling logic | 🟢 Integrated | The Command Center computes real schedules by sending Hive task/event data and the active list's effective settings to a separately-running `PriorityTaskManager.API` instance, not mock data (tracked by issue #47). By default it calls the unauthenticated `/api/local/schedule` route; when built with `--dart-define=PTM_DEV_AUTOLOGIN=free\|subscriber` it instead logs in as a seeded dev account and calls the authenticated, Subscription-gated `/api/schedule` route (see [WORKFLOW.md](WORKFLOW.md)). The client no longer spawns this process itself; the API must already be running. |
...
| Networked API | 🔴 Not yet integrated | No login screen or API-backed repository for tasks/lists/events yet; the client only reaches the schedule-compute routes on a separately-running API instance (tracked by issue #44). A dev-only, define-gated auto-login (`PTM_DEV_AUTOLOGIN`) exists purely to exercise the authenticated `/api/schedule` route during development, not as a substitute for a real login screen. |
...
- `PriorityTaskManager.API` exposes authenticated REST endpoints (`/api/tasks`, `/api/lists`, `/api/events`) for CRUD, an unauthenticated, stateless `/api/local/schedule` endpoint that computes a real Gold Panning (or constraint-optimization) schedule from posted task data without persisting anything server-side, and an authenticated, Subscription-tier-gated `/api/schedule` endpoint running the same stateless computation for callers with a `Subscription`-tier JWT claim.
- `PriorityTaskManager.Flutter/` is a local-only (guest/offline) web + Windows desktop client with its own task/list/event/settings CRUD, supporting add/edit/complete/delete, dependency management, per-list scheduling overrides, and global defaults. Its single-screen "Three-Pane Command Center" (Left Rail, Center Stage, Right Inspector) computes real schedules — including fixed events and the active list's effective settings — by calling a separately-running local `PriorityTaskManager.API` instance rather than using mock data (tracked by issue #47). The client does not start or manage this API process itself (see [WORKFLOW.md](WORKFLOW.md) for how to run it during development). No login or API-backed repository yet (tracked by issue #44).
