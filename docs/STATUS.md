...
- The Left Rail's Archive nav item opens a real dialog (issue #62) listing archived tasks with a per-task Restore action, backed by `getArchivedTasks()`/`restoreArchivedTask()` on `TaskRepository` (`ApiTaskRepository` only). Archive is Authenticated-only, consistent with other online-only features: Guests get an in-app message instead of the dialog, and `LocalTaskRepository`'s archive methods fail fast (`UnsupportedError`) since the UI never calls them for a Guest session. If a task's original list no longer exists, restoring prompts the user to pick a target list before retrying.
