...
- Task/list/event CRUD and LLM-assisted intake remain free and do not require a subscription; there are exactly two account tiers, Free and Subscription (see [VISION.md](VISION.md)). Free-tier intake is protected by a lower usage quota (not a hard paywall) plus the same abuse-prevention request/traffic rate limiter applied to both tiers; Subscription gets a materially higher or unlimited quota.
